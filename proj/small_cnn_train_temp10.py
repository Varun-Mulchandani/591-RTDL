...
if training_required:
    ...
    torch.save(net, 'small_cnn_temp10/net.pt')

else:
    net = torch.load('small_cnn_temp10/net.pt')
...
if training_required:
    ...
    print('Finished Training')
    torch.save(encoder1_prune3, 'small_cnn_temp10/prune1.pt')
else:
    encoder1_prune3 = torch.load('small_cnn_temp10/prune1.pt')
...
if training_required:
    ...
    for epoch in range(25):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            net_outputs = net(inputs)
            loss = criterion(outputs, labels)
            other_loss = other_criterion(F.log_softmax(outputs/50), F.softmax(net_outputs/50))
            # print(other_loss)
            loss += other_loss
            loss.backward()
    ...
    print('Finished Training')
    torch.save(encoder1_prune4, 'small_cnn_temp10/prune2.pt')
else:
    encoder1_prune4 = torch.load('small_cnn_temp10/prune2.pt')
